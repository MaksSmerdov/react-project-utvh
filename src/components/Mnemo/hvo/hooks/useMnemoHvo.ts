...
type HvoKey = keyof typeof apiConfigs;

interface Config<K extends HvoKey> {
  config: (typeof apiConfigs)[K];
}

type Data<K extends HvoKey> = (typeof apiConfigs)[K]['defaultData'];

const useMnemoHvo = <K extends HvoKey>({ config }: Config<K>) => {
